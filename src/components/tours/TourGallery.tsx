...
export default function TourGallery({ images, title }: TourGalleryProps) {
  ...
  return (
    <section className="mb-6 sm:mb-8">
      {/* 1. Main Display Area */}
      <div className="relative group w-full aspect-[4/3] sm:aspect-[16/10] md:aspect-[16/9] max-h-[550px] overflow-hidden rounded-2xl bg-slate-950 border border-slate-200/80 dark:border-slate-800 shadow-lg">
        ...
        >
          {allImages.map((img, idx) => (
            <div 
              key={idx} 
              className="relative w-full h-full shrink-0 snap-center flex items-center justify-center overflow-hidden bg-slate-900"
            >
              {/* Blurred Background Image (لإعطاء منظر احترافي ومنع الفراغات بدون قص الصورة) */}
              <Image
                src={img}
                alt=""
                fill
              ...
              <Image
                ...
                priority={idx === 0}
                sizes="(max-width: 1024px) 100vw, 1200px"
                className="object-contain md:object-cover transition-transform duration-300"
              />
            </div>
          ))}
        </div>
        ...
        {/* Counter Badge */}
        <div className="absolute bottom-3 right-3 z-20 px-3 py-1 rounded-full bg-black/60 text-white backdrop-blur-md text-xs font-medium border border-white/10">
          {activeIndex + 1} / {allImages.length}
        </div>
      </div>
